...
onMounted(() => {
  request.get("/menu.all").then((res) => {
    menus.value = (res.data.data?.menus || [])
      .filter((el) => el.hidden_on_menu !== true)
      .map((el) => {
        if (el.children) {
...
<template>
  <div class="min-h-screen">
    <el-container class="min-h-screen bg-gary-100">
      <div
        class="light:text-light-header-text light:bg-light-header-bg dark:bg-dark dark:text-white h-60px fixed w-full px-4 box-border border-solid border-0 border-b-1px border-gray-300"
      ...
      <div class="flex flex-row flex-1 bg-gray-100">
        <div
          class="w-200px dark:bg-dark-800 light:bg-white fixed h-screen overflow-y-auto hidden-scroll-bar top-60px border-0 border-solid border-r-1px border-gray-200"
          style="z-index: 98"
        >
          <Menu :menus="menus" />
