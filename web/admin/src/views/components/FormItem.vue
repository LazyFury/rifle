<template>
  ...
  <!-- password  -->
  <ElInput @change="handleUpdate" v-model="value" v-if="field.type == 'password'" type="password" :placeholder="field.placeholder"> </ElInput>
  <!-- select  -->
  ...
  <!-- select multi  -->
  <ElSelect v-if="field.type == 'select-multi'" filterable @change="handleUpdate" v-model="value" multiple :placeholder="field.placeholder">
    <ElOption v-for="item in options" :key="item.value" :label="item.label" :value="item.value"></ElOption>
  </ElSelect>

  <!-- switch -->
  <ElSwitch @change="handleUpdate" v-model="value" v-if="field.type == 'switch'" :active-text="field.checkedChildren"> </ElSwitch>
  <!-- checkbox only one -->
  <ElCheckbox @change="handleUpdate" v-model="value" v-if="field.type == 'checkbox' && !field.multiple" :label="field.placeholder"> </ElCheckbox>
  ...
  <!-- radio-button-group  -->
  <ElRadioGroup @change="handleUpdate" v-model="value" v-if="field.type == 'radio-button-group'">
...
export default {
  ...
  methods: {
    ...
    getOptions() {
      ...
      if (["select", "select-multi", "radio-button-group"].includes(this.field.type)) {
        level = 1
      }
